...
TRAIN_RATIO = 0.8
VAL_RATIO = 0.1  # train / val / test are chronological
OUTPUT_DIR = "artifacts/tier2_player_embedding"
...
def build_player_index(df: pd.DataFrame) -> Dict[str, int]:
    ...
    # reserve 0 for unknown, start players at 1
    mapping = {p: i + 1 for i, p in enumerate(players)}
    return mapping


# -------------------------------------------------
# Synthetic inference helpers
# -------------------------------------------------
...
class SportEmbeddingNet(nn.Module):
    def __init__(self, n_players: int, n_features: int, embed_dim: int = 16):
        ...
        input_dim = n_features + embed_dim * 4  # p1, p2, diff, prod
        ...
        self.head = nn.Linear(prev, 2)  # diff, total
...
def train_one_sport_model(
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx in iterate_minibatches(len(df_train), BATCH_SIZE):
            ...
            loss_diff = loss_fn(out[:, 0], y_train[batch_idx, 0])
            loss_total = loss_fn(out[:, 1], y_train[batch_idx, 1])
...
# -------------------------------------------------
@dataclass
class Tier2Predictor:
    models: Dict[str, SportEmbeddingNet]
    ...
    def predict_pair(self, player1: str, player2: str) -> dict:
        ...
        for sport in SPORTS:
            ...
            model.eval()
            with torch.no_grad():
                out = model(p1_idx, p2_idx, x_num).cpu().numpy()[0]
...
def main():
    ...
    n = len(df)
    split_train = int(n * (TRAIN_RATIO - VAL_RATIO))
    ...
    for sport in SPORTS:
        ...
        model, pack = train_one_sport_model(
            sport=sport,
            df=df,
            rows=rows,
        ...
        print(f"\n{sport} TEST RESULTS")
        print("Diff MAE:", pack["diff_mae"])
        print("Total MAE:", pack["total_mae"])

        models[sport] = model
        model_meta[sport] = pack

        test_rows = pack["rows_test"]
        pred_diff = np.array(pack["pred_diff"])
        pred_total = np.array(pack["pred_total"])
            ...
            if sport in ["TT", "BD", "SQ"]:
                ...
                if PREDICT_TENNIS_INDEPENDENTLY:
                    ...
                    s1_hat, s2_hat = decode_tennis_score(
                        pred_diff[i], pred_total[i], running_diff_before_tn
                    )
    ...
    # save package
    serializable_meta = {}
    for sport, meta in model_meta.items():
        serializable_meta[sport] = {
            k: v for k, v in meta.items() if k not in ["state_dict"]
        }

    save_blob = {
        "feature_cols": feature_cols,
        "player_to_idx": player_to_idx,
        "model_meta": serializable_meta,
        "state_dicts": {sport: models[sport].state_dict() for sport in models},
        "config": {
            "embed_dim": EMBED_DIM,
    ...
    torch.save(save_blob, outdir / "tier2_package.pt")
        ...
        json.dump(
            {
                ...
                "match_metrics": {
                    "total_diff_mae": float(
                        mean_absolute_error(true_diff, pred_diff)
                    ),
                    "winner_accuracy": float(
                        ((pred_diff > 0) == (true_diff > 0)).mean()
                    ),
                },
            },
            f,
    ...
    print(f"\nSaved Tier 2 package to: {outdir}")
