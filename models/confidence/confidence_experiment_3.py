...
OUTPUT_DIR = "models/confidence/artifacts/experiment_3"
...
def knn_consistency_query(
    ...
    conf = consistency_confidence(nn_targets)

    return {
        "consistency_confidence": float(conf),
...
def run_experiment():
    ...
    work = (
        df[FEATURE_COLS + [TARGET_COL, "p1_name", "p2_name"]]
        .copy()
        .dropna()
    ...
    for _, row in test_df.iterrows():
        ...
        # geometric confidence from historical local consistency
        q = build_query_from_row(row)
        conf_pack = knn_consistency_query(tree, y_train, q, K)
        ...
        conf_list.append(conf_pack["consistency_confidence"])
    ...
    conf_list = np.asarray(conf_list, dtype=float)
    ...
    conf_s = spearman_corr(conf_list, abs_err)
    conf_q = quartile_error_summary(conf_list, abs_err)
    ...
    print("\n=== CONFIDENCE: LOCAL OUTCOME CONSISTENCY ===")
    print("Spearman(confidence, abs_error):", conf_s)
    print("Low-confidence quartile MAE:", conf_q["low_q_mae"])
    print("High-confidence quartile MAE:", conf_q["high_q_mae"])
    ...
    summary = {
        ...
        "confidence_metrics": {
            "consistency_confidence": {
                "spearman_conf_vs_abs_error": conf_s,
                **conf_q,
            },
            "raw_local_signals": {
...
# -------------------------------------------------
# Optional utility for manual pair confidence
# -------------------------------------------------
def score_pair_confidence(player1: str, player2: str):
    df = read_data(DATA_PATH)
    inference_state = load_inference_state(INFERENCE_STATE_PATH)
    ...
    tree = KDTree(X_train)
    q = build_query_from_players(inference_state, player1, player2)
    return knn_consistency_query(tree, y_train, q, K)
