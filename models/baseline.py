...
# If True, TN is predicted like a normal independent sport.
# If False, TN is cut short using racketlon stop rules.
...
# -------------------------------------------------
# Build stats from training only
# -------------------------------------------------
...
def decode_full_game_score(
    ...
    """
    Decode a completed sport into a legal 21-x or x-21 scoreline,
    using both predicted diff and predicted total.
    """
    pred_diff = float(
...
def decode_tennis_score(
    ...
    """
    TN decoder with racketlon stop-rule truncation.
    """
    pred_diff = float(
        np.clip(pred_diff, -MAX_DIFF_PER_SPORT, MAX_DIFF_PER_SPORT)
    )
...
def fit_models(df: pd.DataFrame, X: pd.DataFrame, targets, split: int):
    ...
    for s in SPORTS:
        ...
        pred_diff = np.clip(pred_diff, -21, 21)
        pred_total = np.clip(pred_total, 0, 42)
